  ...
  {inquirer.prompt([
    {
     ...
     const fs = require('fs');
      fs.readFile(newFilePath, (err, data) => { 
        ...
        datafromtextfile = newstring.split(",");
if (datafromtextfile[0] === 'spotify-this-song'){
  spotify.search({ type: 'track', query: datafromtextfile[1] }, function(err, data) {
    if (err) {
      return console.log('Error occurred: ' + err);
    }
...
else if (datafromtextfile[0] === 'movie this'){
  axios({
    method: 'get',
    url:"http://www.omdbapi.com/?apikey=daba8cc8&"+"s=" + datafromtextfile[1],
  }).then(function (response) {
    console.log("Movie Title: "+response.data.Search[0].Title);
    console.log("Year: "+response.data.Search[0].Year);
  ...
  axios({
    method: 'get',
    url: "https://rest.bandsintown.com/artists/" + datafromtextfile[1] + "/events?app_id=codingbootcamp",
  }).then(function (response) {
